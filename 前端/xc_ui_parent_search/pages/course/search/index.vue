...
<script>
  ...
  export default {
    ...
    async asyncData({ store, route }) {//服务端调用方法
      ...
      //向java微服务发起请求搜索课程
      //请求搜索服务，搜索服务
      let { data:course_data } = await search_course(page,2,route.query);
      //查询分类
      let { data:category_data} = await sysres_category()
      if (course_data.success &&　course_data.queryResult ) {
        //全部分类
    ...
    async mounted(){
      // 测试
      let page = 1
      let { data:course_data} = await search_course(page,2);
